											   ...
											   VkDeviceSize&  dstBufferOffset) const
{
	...
	deUint8		 shortBufferData[shortBufferDataSize];
...
size_t BitstreamBufferImpl::GetMaxSize() const
{
	return m_bufferSize;
}

size_t BitstreamBufferImpl::GetOffsetAlignment() const
{
	return m_bufferOffsetAlignment;
}

size_t BitstreamBufferImpl::GetSizeAlignment() const
{
	// REVIEW: Cache this?
...
size_t BitstreamBufferImpl::Resize(size_t, size_t, size_t)
{
	TCU_THROW(InternalError, "Bitstream buffers should never need to be resized in the CTS");
}

deUint8* BitstreamBufferImpl::CheckAccess(size_t offset, size_t size) const
{
	DE_ASSERT(offset + size < m_bufferSize && size > 0);
	DE_UNREF(size);
	auto* bitstreamBasePtr = static_cast<deUint8*>(m_bitstreamBuffer->getAllocation().getHostPtr());
	return static_cast<deUint8*>(bitstreamBasePtr + offset);
}

int64_t BitstreamBufferImpl::MemsetData(deUint32 value, size_t offset, size_t size)
{
	if (size == 0)
	{
		return 0;
...
int64_t BitstreamBufferImpl::CopyDataToBuffer(deUint8* dstBuffer, size_t dstOffset, size_t srcOffset, size_t size) const
{
	if (size == 0)
	{
		return 0;
...
int64_t BitstreamBufferImpl::CopyDataToBuffer(VkSharedBaseObj<VulkanBitstreamBuffer>& dstBuffer, size_t dstOffset, size_t srcOffset, size_t size) const
{
	if (size == 0)
	{
		return 0;
...
int64_t BitstreamBufferImpl::CopyDataFromBuffer(const deUint8* sourceBuffer, size_t srcOffset, size_t dstOffset, size_t size)
{
	if (size == 0)
	{
		return 0;
...
deUint8* BitstreamBufferImpl::GetDataPtr(size_t offset, size_t& maxSize)
{
...
const deUint8* BitstreamBufferImpl::GetReadOnlyDataPtr(size_t offset, size_t& maxSize) const
{
	deUint8* readData = CheckAccess(offset, 1);
	DE_ASSERT(readData);
	maxSize = m_bufferSize - offset;
...
void BitstreamBufferImpl::FlushRange(size_t /*offset*/, size_t size) const
{
	if (size == 0)
	{
		return;
...
void BitstreamBufferImpl::InvalidateRange(size_t /*offset*/, size_t size) const
{
	if (size == 0)
	{
		return;
