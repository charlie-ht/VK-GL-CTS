...
int32_t VideoBaseDecoder::StartVideoSequence (const VkParserDetectedVideoFormat* pVideoFormat)
{
	...
	// other test content. Should look more ino this problem, but for now cheese it by always allocating the maxmimum frame count (for the CTS) of 32.
	// This is inefficient but safe and reliable. Conceptually this should the maximum size of a GOP
	m_numDecodeSurfaces = 32u; // N.B. pVideoFormat->minNumDecodeSurfaces is NOT advertised correctly!
...
int32_t VideoBaseDecoder::DecodePictureWithParameters(MovePtr<CachedDecodeParameters>& cachedParameters)
{
	...
	// pPicParams->decodeFrameInfo.dstImageView = VK_NULL_HANDLE;
...
void VideoBaseDecoder::ApplyPictureParameters(de::MovePtr<CachedDecodeParameters> &cachedParameters)
{
	...
	DE_ASSERT(pOwnerPictureParameters->GetId() <= cachedParameters->currentPictureParameterObject->GetId());
	// TODO: We should probably cache the active video session as part of the cached parameters object during early decode.
...
void VideoBaseDecoder::WaitForFrameFences(de::MovePtr<CachedDecodeParameters> &cachedParameters)
{
	...
	VK_CHECK(m_deviceContext->getDeviceDriver().waitForFences(m_deviceContext->device, 1, &cachedParameters->frameSynchronizationInfo.frameCompleteFence, true, 100 * 1000 * 1000));
...
void VideoBaseDecoder::SubmitQueue(de::MovePtr<CachedDecodeParameters> &cachedParameters)
{
	...
	{
		VK_CHECK(vk.waitForFences(device, 1, &cachedParameters->frameSynchronizationInfo.frameConsumerDoneFence, true, 100 * 1000 * 1000));
	...
	{ // For fence/sync debugging
		...
		else
		{
			...
			{
				VK_CHECK(vk.waitForFences(device, 1, &cachedParameters->frameSynchronizationInfo.frameCompleteFence, true, 100 * 1000 * 1000));
												  ...
												  size_t initializeBufferMemorySize, VkSharedBaseObj<VulkanBitstreamBuffer> &bitstreamBuffer)
{
	...
	if (!(availablePoolNode >= 0)) {
		VK_CHECK(VulkanBitstreamBufferImpl::Create(m_deviceContext,
															m_deviceContext->decodeQueueFamilyIdx(),
		...
		if (enablePool) {
			...
			TCU_CHECK_MSG(nodeAddedWithIndex >= 0, "Could not add the new node to the pool");
		}
	} else {
								 ...
								 VkSharedBaseObj<VkImageResource>& imageResource)
{
	...
	do {
		...
		result = VulkanDeviceMemoryImpl::Create(vkDevCtx.context->getInstanceInterface(),
												...
												vkDeviceMemory);
		if (result != VK_SUCCESS) {
			DE_ASSERT(!"Create Memory Failed!");
			break;
		}
		...
		if (result != VK_SUCCESS) {
			DE_ASSERT(!"BindImageMemory Failed!");
			break;
		}
